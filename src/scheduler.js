  ...
  cron.schedule(cronExpression, async () => {
    try {
      ...
      if (config.startDate) {
        ...
        if (today < startDate) {
          logInfo(`아직 시작일(${startDate})이 아닙니다. 현재: ${today}. 대기 중...`);
          return;
        }
      }
      ...
      if (!imageData) {
        logError('이미지 다운로드 실패', new Error(`인덱스 ${nextIndex}의 이미지를 찾을 수 없습니다.`));
        return;
      }
...
export function scheduleDailyReport() {
  ...
  logInfo(`일일 완독률 보고 스케줄 등록: ${config.completionReportTime} (매일)`);
  ...
  cron.schedule(cronExpression, async () => {
    try {
      ...
      let totalMembers = 0;
      try {
        const chatMemberCount = await bot.telegram.getChatMemberCount(config.telegram.groupChatId);
...
export async function generateAndSendOverallStats() {
  try {
    ...
    const totalCompletions = allDailyStats.reduce((sum, stat) => sum + stat.completed_count, 0);
    const averageRate = totalDays > 0 
      ? (allDailyStats.reduce((sum, stat) => sum + stat.completion_rate, 0) / totalDays).toFixed(1)
      : 0;
